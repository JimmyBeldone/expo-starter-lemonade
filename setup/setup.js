...
const appJsonPathPrefix = process.env.MODE === 'test' ? 'setupCopy/' : '';
...
const updatePackage = async () => {
    ...
    // Replace app.json name and slug
    replace({
        paths: [`${appJsonPathPrefix}app.json`],
        recursive: false,
        regex: /"name": "(.*?)"/,
        replacement: `"name": "${responses.name}"`,
        silent: true,
    });
    ...
    replace({
        ...
        recursive: false,
        regex: /"slug": "(.*?)"/,
        replacement: `"slug": "${responses.name}"`,
        silent: true,
    });
    ...
    async function cleanDeps() {
        const { stdout } = await exec(
            'yarn remove chalk prompts replace rimraf -D',
        );
